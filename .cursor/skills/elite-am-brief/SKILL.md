...
| **Monthly Birthday Gifts** (Daily Triggers) | `agents[].birthdayGift` | `birthday_gift_sql` (**birthday month == report month** AND lifetime Hold % >= `BIRTHDAY_GIFT_MIN_HOLD_PCT` AND trailing-30-day purchase >= `BIRTHDAY_GIFT_MIN_30D_PURCHASE`); **weekly Sunday refresh** via `load_or_refresh_birthday_gift` (caches `data/birthday_gift_weekly.json`, reused Mon-Thu) | `build_birthday_gift_section` (drops locked/SE/TAB; Hold %/LTP/30D off the row; gated gift draft via `am_brief_ticket_drafts.build_birthday_gift_ticket_draft`, `gift_month` fills the greeting) | `views/birthdayGift.ts` | — |
| **Responsiveness** (Daily Triggers) | `agents[].responsiveness` | `ticket_inactivity_sql` (last ticket created/updated older than `TICKET_INACTIVITY_DAYS` (90); never-ticketed accounts are not surfaced) | `build_responsiveness_section` (drops locked/SE/TAB; LTP/Hold %/30D off the shared `enrich_aids_sql` batch; no draft) | `views/responsiveness.ts` | — |
