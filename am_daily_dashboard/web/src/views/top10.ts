...
import { aidHtml, holdHtml } from "./../cells";
...
function offerCell(p: Record<string, unknown>): string {
  ...
  const qtyHint = qty > 1 ? `<span class="t-tertiary t-small"> x${qty}</span>` : "";
...
export function viewTop10(): string {
  return tableCard({
    ...
    showSearch: false,
    headers: ["#", "AID", "Name", "Purchased $", "Top Offer", "Price",
              "Frequent 30d", "Max Purchase 30D", "LTP", "Hold"],
    align: ["center", "left", "left", "right", "left", "right", "right", "right", "right", "right"],
    markerCol: 3,
    ...
    renderRow: (p) => [esc(p.rank), aidHtml(p), `<span class="top10-name">${esc(p.name)}</span>`,
      ...
      offerCell(p), priceCell(p),
      frequent30Cell(p),
      `<span class="top10-money">${esc(p.maxPurchase30d || "—")}</span>`,
      `<span class="top10-money">${esc(p.lifetimePurchase || "—")}</span>`,
